...
export const useLayoutMenu = defineStore("layout-menu", () => {
  ...
  const handleAccordionMode = (innerOpenKeys: string[]) => {
    ...
    if (latestOpenKey) {
      if (!rootSubmenuKeys.includes(latestOpenKey)) {
        // Compare with previous item to see if they are at the same level, if so remove the previous item
  ...
  const handleSelectedKeys = (val: string[]) => {
    // If clicking on external menu, we don't need to set it as active state
    const path = val[0];
    if (!isUrl(path))
