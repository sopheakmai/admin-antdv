...
// Generate menu info from local static routes
export function genRoutes(routes: RouteRecordRaw[], parent?: MenuDataItem) {
  const menuData: MenuData = [];
  routes.forEach((route) => {
    let path = route.path;
    if (!path.startsWith("/") && !isUrl(path)) {
      // Check if current path starts with /, if not, it means the current route address is incomplete
      if (parent)
        path = `${parent.path}/${path}`;
      else
        path = `/${path}`;
    }
    // Check if name exists, if not, automatically add a custom name for easier implementation of keep-alive feature, name is required
    if (!route.name)
...
/**
 * Request backend data to get menu information, default data is flattened and needs to be organized into tree structure
 */
